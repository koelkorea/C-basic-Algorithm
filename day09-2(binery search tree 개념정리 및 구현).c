...
//   # 이진검색 트리의 특징 
//     1. 같은 값을 갖는 노드는 없음
//     2. 중위 순회를 하면 최소값 ~ 최대값의 전체수를 오름차순으로 출력할 수 있음
...
//        : 삭제노드와 가장 근사 값을 가진 노드를 삭제노드 위치로 가져옴 (왼쪽에서 가장 큰 값 or 오른쪽에서 가장 작은 값)
//          -> 단! 구현을 쉽게 하기 위해서는 link구조를 변환할 필요 없는 쪽으로 가도록하자
...
// 트리구조 삭제 메서드 (= 기본적으로 트리구조는 재귀적 방법을 사용)
