    ...
    if (direction == ascending_order) {
        ...
        while(left_start <= right_start){
            ...
            if (left_start == right_start) {
                ...
                //printf("-> 기준값 %d을 가지고 있던 arr[%d]의 위치를 분기점 배열[%d]의 값과 스왑", standard, start, left_start);
                //printf("arr[%d] : %d <-> arr[%d] : %d!\n\n", start, arr[start], left_start, arr[left_start]);
                ...
                if (left_start == array_length && right_start == array_length) {
                    ...
                    printf("(부분 정렬 완료) 현재 left_start == right_start == %d 해당 부분배열의 배열은 완성되었고, 더 부분으로 가를 이유가 없습니다.\n\n", array_length);
            ...
            //}
                ...
                printf("(교환값 발견후!) left_start -> %d, right_start -> %d\n\n", left_start, right_start);

                if (left_start <= right_start) {
                    printf("->> left : %d <= right : %d 이므로..  arr[left_start] : %d <-> arr[right_start] : %d \n\n", left_start, right_start, arr[left_start], arr[right_start]);
