...
void push(StackType* stack, Stack_DoublelistElement item) {
    ...
    if (result == 0) {
        ...
        stack = DoubleInsertLast(stack->head, item);
...
Stack_DoubleListNode* DoubleInsertLast(Stack_DoubleListNode* head, Stack_DoublelistElement item) {
    ...
    Stack_DoubleListNode* insert_last_node = (DoubleListNode*)malloc(sizeof(DoubleListNode));
    ...
    if (head->Rlink != head) {
        ...
        head->Rlink = insert_last_node;
        insert_last_node->Llink = head->Llink;
    }
