...
int interpolation_search(int* arr, int key, int low, int high) {
    ...
    // 보간 검색 결과인 result를 반올림한 정수를 인덱스의 시작점으로.. = 보간검색 공식을 통해 얻은 주소값 search_index의 상대위치
    int search_index = (int)(result + 0.5);

    // search_index 조정되는 가장 낮은 주소와 높은 주소 사이에 있으면 계속 진행
    //  -> 조건은 이렇게 잡아도 무한루프시 탈출 할 방법은 1번째 if 조건에 둠
    while (search_index >= low || search_index < high) {

        // 현재 가장 낮은 주소와 높은 주소가 붙어있으면 실패로 간주 (= 그 정도까지 왔는데 못 찾았다는건, 결국 값이 없어 포인터가 갈팡질팡한다는 의미)
        if (low + 1 == high) {
            // 검색에 실패했을 경우 -1 반환
            return -1;
        }
        ...
        // 배열의 시작주소 + search_index를 역참조한 값이 key보다 크다면?
        ...
        // 배열의 시작주소 + search_index를 역참조한 값이 key보다 작다면?
