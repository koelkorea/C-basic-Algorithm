...
// 방법1) 색인 순차 탐색 정적배열을 사용하여 구현 (주어진 index배열 길이 -> index 데이터의 최적화 간격 도출하는 방법)
#include <stdio.h>
#include <stdlib.h>				//	srand(), rand() 함수 사용 가능하게 함
#include <time.h>
#define INDEX_REFERENCE_LENGTH 10   // [방법2 핵심] 애초부터 원하는 목차 index 데이터 길이를 사용
...
void indexSearch(int target_array[], int target_array_length, int key) {
    ...
    for (int i = 0; i < INDEX_REFERENCE_LENGTH; i++) {
        ...
        // index 배열의 index_reference_value 값이 0인 경우, index 정보를 기입하는 도중 interval이 +1 되는 바람에 추가되지 않았다는 의미이니, 의미없이 순회하는 반복문을 끊어준다
...
void makeIndexTable(int target_array[], int target_array_length) {
    ...
    // [방법2의 핵심] 타겟 배열의 길이 / index 배열의 길이 = 각 index의 구간을 얼마로 설정해야 할지 계산 (= 최적화 된 구간 도출)  
    ...
    // index 배열 정보를 채우기 시작 (이미 최적화 된 index 간격을 도출함)
    for (int i = 0; i < INDEX_REFERENCE_LENGTH; i++) {
