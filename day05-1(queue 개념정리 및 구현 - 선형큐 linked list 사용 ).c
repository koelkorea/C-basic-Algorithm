...
// DeQueue에 새로운 노드 생성 및 추가(= int direction의 방향에 따라 맨 앞, 뒤 선택 가능)  
void Endeque(LinkedQueueType* queue, QueueElement data, int direction);

// 현재의 Queue에 기존에 존재한 첫번째 Node or 마지막 노드 삭제 (= int direction의 방향에 따라 맨 앞, 뒤 선택 가능)  
...
void Endeque(LinkedQueueType* queue, QueueElement data, int direction) {
    ...
    if (QueueIsEmpty(queue) == 0) {
        ...
        if (direction == FIRST) {
            ...
            // 추가되는 노드의 link값을 기존 1번째 노드의 주소값으로 지정
            Newnode->link = queue->front;

            // queue의 앞노드의 주소만 새로 만들어진 Newnode로 지정
            queue->front = Newnode;

            printf_s("DeQueue의 첫번째 값인 %d을 입력완료\n", queue->front->data);
...
QueueElement Dedeque(LinkedQueueType* queue, int direction) {
    ...
    if (QueueIsEmpty(queue) == 0) {
        ...
        // 방향을 마지막 추가로 잡을 경우
            ...
            // 새롭게 마지막 노드가 될 녀석이 될 변수
            QueueNode* newLastNode = queue->front;

           // 제거 후 마지막 노드가 될 녀석을 찾기 위한 while문 (link가 가르키는 녀석이 deleteNode의 주소값인 노드의 주소)
            while (newLastNode->link != deleteNode) {

                newLastNode = newLastNode->link;
            }

            printf_s("queue의 마지막 값인 %d을 삭제완료\n", queue->rear->data);

            // queue의 끝노드의 주소값을 기존 마지막에서 2번째 노드의 값으로 변경
            queue->rear = newLastNode;

            // 기존 마지막에서 2번째 노드의 link값도 NULL로 변경
