...
int interpolation_search(int arr[], int key, int low, int high) {
    ...
    if (low <= high) {
        ...
        // 배열[search_index]에 위치한 값이 key보다 크다면? -> high의 값을 search_index - 1 (해당 주소값보다 1작은 주소에 위치한 주소)로 두고 다시 돌림
        else if (arr[search_index] > key) {

            return interpolation_search(arr, key, low, search_index - 1);
        }
        // 배열[search_index]에 위치한 값이 key와 작다면? -> low의 값을 search_index + 1 (해당 주소값보다 1높은 주소에 위치한 주소)로 두고 다시 돌림
        else if (arr[search_index] < key) {
...
int main() {
    ...
    while (1) {
        ...
        if (input <= 0 || input > ary_test[ARRAY_LENGTH - 1]) {
